 ...
 * Only the bottom 9 bits of the address are used, so those over 0xA200 will be
 * echoes of the region 0xA000-0xA1FF.
 * Returns 0xFF if RAM is not accessible or if the address is invalid. */
uint8_t Mbc2::read_ram(uint16_t address) const {
    ...
    return ram[0][address & (RAM_BANK_SIZE / 16 - 1)] & 0xF;
}

/* Write the trailing 4 bits of the given 8 bit value to the given address.
 * Fails if RAM is not accessible or if the address is invalid. */
void Mbc2::write_ram(uint16_t address, uint8_t value) {
    ...
    ram[0][address & (RAM_BANK_SIZE / 16 - 1)] = value & 0xF;
}